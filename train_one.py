...
((fw_outputs, bw_outputs), (_, _)) = bi_rnn(cell_fw=GRUCell(HIDDEN_SIZE),
                                                                                 cell_bw=GRUCell(HIDDEN_SIZE),
                                                                                 inputs=batch_embedded,
                                                                                 sequence_length=length(batch_embedded),
                                                                                 dtype=tf.float32)
...
print("rrn)ourput size:  ",rnn_outputs)
tf.summary.histogram('RNN_outputs', rnn_outputs)

# Attention layer
with tf.name_scope('Attention_layer'):
    # attention_output, alphas = attention(rnn_outputs, ATTENTION_SIZE, return_alphas=True)
    attention_output = attention1(rnn_outputs,ATTENTION_SIZE)
    print("attention_output is shape: ",attention_output)
